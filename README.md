...
We analyzed an existing dataset including 17 attributes possibly linked to obesity to identify the most important factors related to different obesity levels (underweight, normal, overweight I, overweight II, obesity I, obesity II and obesity III). Then, a logistic regression model and a decision tree classifier wew tested to predict if one is overweight/obese or not based on the most influential attributes associated to overweight/obesity identified at the first step (number of meals per day, daily water consumption, physical activity frequency per week, time using technology per day, age and height).
